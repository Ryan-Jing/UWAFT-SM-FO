...
void ComponentClass::computeSkModelOutputs()
{
  ...
  const ExtY_simple_test_T &simple_test_Y = simple_test_Obj.getExternalOutputs();
...
void ComponentClass::computeSkModelProps()
{
  ...
  P_simple_test_T &simple_test_P = simple_test_Obj.simple_test_P;
  m_skModelParams.emplace_back(
    "gain_Gain",
    0,
    1,
    SkTypeId::t_double,
    "2.0",
    &simple_test_P.gain_Gain,
    [] (MAPSProperty& mapsProp, void *data) -> bool {
    return parseData< SkTypeId::t_double >(mapsProp, *static_cast<real_T*>(data));
  }
  );
}
...
{
  simple_test_Obj.initialize();
}

void ComponentClass::doSkModelStep()
{
  simple_test_Obj.setExternalInputs(&simple_test_U);
  simple_test_Obj.step();
}

void ComponentClass::terminateSkModel()
{
  simple_test_Obj.terminate();
}
