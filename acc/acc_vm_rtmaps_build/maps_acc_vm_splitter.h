#ifndef maps_acc_vm_splitter_h_
#define maps_acc_vm_splitter_h_
#ifdef _MSC_VER
#if _MSC_VER < 1900
...
#include <vector>
#include <maps.hpp>
#include "./acc_vm_rtmaps_tlc_helper.h"
#include "./acc_vm_rtmaps_tlc_types.h"
#define ComponentClass                 maps_acc_vm_splitter
#define ComponentName                  "acc_vm_splitter"
#define ComponentVersion               "2.12"

class ComponentClass : public MAPSComponent
{
 private:
  friend class acc_vm_rtmaps_tlc_helper::DynamicInputModel;
  template <typename TComponent, typename TContainer, typename TLambda>
    friend
    void acc_vm_rtmaps_tlc_helper::fillEnumPropertyWithContainerValues
    (TComponent* comp,
 ...
 public:
  struct OutputWrapper
  {
    ...
    template <typename TAllocCallable, typename TWriteCallable>
      OutputWrapper(
                    MAPSOutput* output_, const acc_vm_rtmaps_tlc_types::Field&
                    field_,
 ...
 private:
  ...
  OutputWrapper makeStructOutputWrapper(MAPSOutput* output, const
    acc_vm_rtmaps_tlc_types::Field& field);
  OutputWrapper createOutput(const acc_vm_rtmaps_tlc_types::Field& field);
 private:
  std::string m_input_type;
  std::string m_output_selection_regex;
  bool m_firstTime;
  const acc_vm_rtmaps_tlc_types::Struct* m_inputStruct = nullptr;
  acc_vm_rtmaps_tlc_helper::DynamicInputModel m_input;
