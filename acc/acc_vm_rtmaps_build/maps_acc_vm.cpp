...
{
  const MAPSTypeFilterBase MAPSFilter_ACCStatusBus =
    MAPS_FILTER_USER_STRUCTURE(ACCStatusBus);
  const MAPSTypeFilterBase MAPSFilter_CurrentStateBus =
    MAPS_FILTER_USER_STRUCTURE(CurrentStateBus);
}
...
// image
  MAPS_INPUT("i_sampling_iplimage", MAPS::GetTypeFilter<IplImage>(), MAPS::
             SamplingReader)
// struct --------------------------------------------------------------------------------------
  MAPS_INPUT("i_fifo_ACCStatusBus", MAPSFilter_ACCStatusBus, MAPS::FifoReader)
  MAPS_INPUT("i_sampling_ACCStatusBus", MAPSFilter_ACCStatusBus, MAPS::
             SamplingReader)
  MAPS_INPUT("i_fifo_CurrentStateBus", MAPSFilter_CurrentStateBus, MAPS::
             FifoReader)
  MAPS_INPUT("i_sampling_CurrentStateBus", MAPSFilter_CurrentStateBus, MAPS::
             SamplingReader)
...
// image
  MAPS_OUTPUT("o_iplimage", MAPS::GetTypeInfoValue<IplImage>(), nullptr, nullptr,
              0)
// struct --------------------------------------------------------------------------------------
  MAPS_OUTPUT_USER_STRUCTURES_VECTOR("o_ACCStatusBus",ACCStatusBus,0)
  MAPS_OUTPUT_USER_STRUCTURES_VECTOR("o_CurrentStateBus",CurrentStateBus,0)
...
// model params
  ...
  MAPS_PROPERTY("p_bool", false, false, true)
  MAPS_PROPERTY("p_string", "", false, true)
...
#define TypeID(type)                   TypeID_Wrapper<type>::value
  ...
  {
    if (skInput != nullptr) {
      skInput->m_writeToModelInput(TypeID(TData), data, skInput->m_dataPtr,
        elementCount);
    }
  }
  ...
  {
    if (skOutput != nullptr) {
      skOutput->m_readFromModelOutput(TypeID(TData), &data, skOutput->m_dataPtr,
        1);
    } else {
...
{
  m_skModelInputs.clear();
  m_skModelInputs.reserve(G_MODEL_INPUT_COUNT);
  m_skModelInputs.emplace_back(
    ...
    SkTypeId::t_struct,
    "ACCStatusBus",
    static_cast<int>(sizeof( ACCStatusBus )),
    &acc_vm_U.ACC_Inputs,
        ...
        size_t elementCount)
  {
    ...
    // TODO
    copyData(
             static_cast<data_pointer_type>(mapsInPtr),
             static_cast<ACCStatusBus*>(data),
             elementCount
             );
  }
  ...
  m_skModelInputs.emplace_back(
    ...
    SkTypeId::t_struct,
    "CurrentStateBus",
    static_cast<int>(sizeof( CurrentStateBus )),
    &acc_vm_U.Current_State_Bus,
    [] (const TypeID_Value mapsTypeId, const void* mapsInPtr, void *data, const
        size_t elementCount)
  {
    (void) mapsTypeId;
    using data_pointer_type =
      ...
      typename std::remove_pointer<
      typename std::decay<
      decltype( &acc_vm_U.Current_State_Bus )
      >::type
      >::type
      >::type
      >::type
    ...
    copyData(
             static_cast<data_pointer_type>(mapsInPtr),
             static_cast<CurrentStateBus*>(data),
             elementCount
             );
  }
...
{
  m_skModelOutputs.clear();
  m_skModelOutputs.reserve(G_MODEL_OUTPUT_COUNT);
  const ExtY_acc_vm_T &acc_vm_Y = acc_vm_Obj.getExternalOutputs();
  m_skModelOutputs.emplace_back(
    "ACCCurrentState",
    0,
    1,
    SkTypeId::t_uint8,
    ...
    &acc_vm_Y.ACCCurrentState,
    [] (const TypeID_Value mapsTypeId, void* mapsOutPtr, const void *data, const
        size_t elementCount)
  {
    copyData(
             mapsTypeId,
...
std::mutex ComponentClass::s_propertiesMutex;

void ComponentClass::computeSkModelProps()
{
  m_skModelParams.clear();
  m_skModelParams.reserve(G_MODEL_PARAM_COUNT);
}
  ...
  , m_skModelStepSize(static_cast<MAPSDelay>(1000000.0 * 0.2))
{
  ...
  computeSkModelProps();
  computeComponentIOs(m_skModelInputs , m_componentInputs );
  computeComponentIOs(m_skModelOutputs, m_componentOutputs);
}
...
void ComponentClass::Set(MAPSProperty &prop, const MAPSString &value)
{
  MAPSComponent::Set(prop, value);
  std::unique_lock<std::mutex> lock(s_propertiesMutex);
  (void)value;              // paramInfo.m_writeToModelParam retrieves the value
  for (auto &paramInfo : m_skModelParams) {
    if (paramInfo.m_name == std::string(prop.ShortName().Beginning())) {
      if (!paramInfo.m_writeToModelParam(prop, paramInfo.m_dataPtr)) {
...
void ComponentClass::Set(MAPSProperty &prop, bool value)
{
  MAPSComponent::Set(prop, value);
  std::unique_lock<std::mutex> lock(s_propertiesMutex);
  (void)value;              // paramInfo.m_writeToModelParam retrieves the value
...
void ComponentClass::createProperties()
{
  switch (m_executionMode)
  {
   ...
   case ExecMode::PeriodicSamplingWhilePostProcessing:
    ...
    DirectSet(Property("executionPeriod"), m_skModelStepSize);
    break;
...
void ComponentClass::createProperties_modelParams()
{
  ...
  for (const auto& paramInfo : m_skModelParams) {
...
void ComponentClass::createProperties_forcedInputValues()
{
  ...
  const size_t firstInputIdx = (m_executionMode == ExecMode::
    ...
    ? 1
    : 0;
  for (size_t inputIdx = firstInputIdx; inputIdx < m_componentInputs.size();
       ++inputIdx) {
    auto& info = m_componentInputs[inputIdx];
    switch (info.m_typeId)
    {
     case TypeID(double ):
     case TypeID(float ):
  ...
  if (forcedInputCount >= m_componentInputs.size()) {
    hasErrors = true;
...
void ComponentClass::createInputs()
{
  ...
  {
   case ExecMode::Periodic:
    for (const auto& info : m_componentInputs) {
   ...
   case ExecMode::TriggeredByFirstInput:
    ...
    {
      newInput("fifo", m_componentInputs.front());
    }

    // remaining data inputs
    for (size_t idx = 1; idx < m_componentInputs.size(); ++idx) {
      newInput("sampling", m_componentInputs[idx]);
    }
    break;
...
void ComponentClass::createOutputs()
{
  m_outputMap.clear();
  m_dataOutputs.clear();
  for (const auto& info : m_componentOutputs) {
     ...
     case TypeID(IplImage):
      {
        ...
        imageInfo.m_size ->m_readFromModelOutput(TypeID(int32_t), imageSize ,
          imageInfo.m_size->m_dataPtr, 2);
        imageInfo.m_channelSeq->m_readFromModelOutput(TypeID(int8_t ), chSeq ,
          imageInfo.m_channelSeq->m_dataPtr, 4);
        chSeq[4] = '\0';
...
{
  acc_vm_Obj.initialize();
}

void ComponentClass::doSkModelStep()
{
  acc_vm_Obj.setExternalInputs(&acc_vm_U);
  acc_vm_Obj.step();
}

void ComponentClass::terminateSkModel()
{
  acc_vm_Obj.terminate();
}
   ...
   case TypeID(IplImage):
    {
      ...
      imageInfo.m_size ->m_writeToModelInput(TypeID(int32_t), imageSize ,
        imageInfo.m_size->m_dataPtr, 2);
      imageInfo.m_channelSeq->m_writeToModelInput(TypeID(int8_t) ,
        data.channelSeq, imageInfo.m_channelSeq->m_dataPtr, 4);
   ...
   case TypeID(Struct_t):
    {
      ...
      info.m_data->m_writeToModelInput(info.m_typeId, inElt.DataPointer(),
        info.m_data->m_dataPtr, vectorSizeValue);
      break;
    }
   ...
   case TypeID(Struct_t):
    {
      info.m_data->m_readFromModelOutput(info.m_typeId, outGuard.DataPointer(),
        info.m_data->m_dataPtr, outGuard.VectorSize());
