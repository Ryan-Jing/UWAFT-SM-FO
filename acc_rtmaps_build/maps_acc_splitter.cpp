#include "maps_acc_splitter.h"
#include <algorithm>
#include <cstring>
#include <type_traits>
#include <maps_type_traits.hpp>

using namespace acc_rtmaps_tlc_helper;
...
// bool
  MAPS_OUTPUT("o_bool" , MAPS::GetTypeInfoValue< to_rtmaps_type_t<bool > >(),
              nullptr, nullptr, 0)
// signed ints
  MAPS_OUTPUT("o_int8_t" , MAPS::GetTypeInfoValue< to_rtmaps_type_t<int8_t > >(),
              nullptr, nullptr, 0)
  MAPS_OUTPUT("o_int16_t" , MAPS::GetTypeInfoValue< to_rtmaps_type_t<int16_t > >
              (), nullptr, nullptr, 0)
  MAPS_OUTPUT("o_int32_t" , MAPS::GetTypeInfoValue< to_rtmaps_type_t<int32_t > >
              (), nullptr, nullptr, 0)
  MAPS_OUTPUT("o_int64_t" , MAPS::GetTypeInfoValue< to_rtmaps_type_t<int64_t > >
              (), nullptr, nullptr, 0)
// unsigned ints
  MAPS_OUTPUT("o_uint8_t" , MAPS::GetTypeInfoValue< to_rtmaps_type_t<uint8_t > >
              (), nullptr, nullptr, 0)
  MAPS_OUTPUT("o_uint16_t" , MAPS::GetTypeInfoValue< to_rtmaps_type_t<uint16_t> >
              (), nullptr, nullptr, 0)
  MAPS_OUTPUT("o_uint32_t" , MAPS::GetTypeInfoValue< to_rtmaps_type_t<uint32_t> >
              (), nullptr, nullptr, 0)
  MAPS_OUTPUT("o_uint64_t" , MAPS::GetTypeInfoValue< to_rtmaps_type_t<uint64_t> >
              (), nullptr, nullptr, 0)
// floats
  MAPS_OUTPUT("o_float" , MAPS::GetTypeInfoValue< to_rtmaps_type_t<float > >(),
              nullptr, nullptr, 0)
  MAPS_OUTPUT("o_double" , MAPS::GetTypeInfoValue< to_rtmaps_type_t<double > >(),
              nullptr, nullptr, 0)
// other
...
MAPS_BEGIN_ACTIONS_DEFINITION(ComponentClass)
  MAPS_ACTION2("a_selected_data_type_info", &ComponentClass::
               a_selected_input_type_info, true)
...
{
  DirectSet(Property(P_IDX_INPUT_TYPES), acc_rtmaps_tlc_helper::
            typeListToMapsString_lite( acc_rtmaps_tlc_types::getStructs() ));
  fillInputTypeEnum();
  ...
  m_input.newInput(this, std::string{ "i_" } + m_input_type, MAPS::Structure,
                   m_input_type, MAPS::FifoReader);
  m_inputStruct = & acc_rtmaps_tlc_types::getStruct(m_input_type);
  DirectSet(Property(P_IDX_INPUT_TYPE_INFO), MAPSString
            (m_inputStruct->toString_lite().c_str()));
  m_output_selection_regex = toString(GetStringProperty
    (P_IDX_OUTPUT_SELECTION_REGEX));
  createOutputs();
}

void ComponentClass::fillInputTypeEnum()
{
  using acc_rtmaps_tlc_types::Struct;
  using acc_rtmaps_tlc_types::getStructs;
...
void ComponentClass::createOutputs()
{
  ...
  auto structIt = std::find_if(structVec.begin(), structVec.end(), [this] (const
    Struct& st)
  {
...
template <typename TDataType>
  ComponentClass::OutputWrapper ComponentClass::makePrimitiveOutputWrapper
  (MAPSOutput* output, const acc_rtmaps_tlc_types::Field& field)
{
  return {
    output,
    field,
    ...
    [this, output, field] (const uint8_t* rawStructArray, const size_t
    structCount, const MAPSTimestamp ts) {
      ...
      for (size_t structIdx = 0; structIdx < structCount; ++structIdx, out +=
           field.capacity, rawStruct += m_inputStruct->size) {
...
ComponentClass::OutputWrapper ComponentClass::makeStructOutputWrapper(MAPSOutput*
  output, const acc_rtmaps_tlc_types::Field& field)
{
  return {
    output,
    field,
    [output, field] (const size_t bufferSize) {
      output->AllocOutputBuffer(static_cast<int>(bufferSize * field.size *
      field.capacity));
    },
    [this, output, field] (const uint8_t* rawStructArray, const size_t
    structCount, const MAPSTimestamp ts) {
      ...
      const uint8_t* rawStruct = rawStructArray;
      for (size_t idx = 0; idx < structCount; ++idx, out += (fieldSize *
            field.capacity), rawStruct += m_inputStruct->size) {
        const uint8_t* const rawField = rawStruct + field.offset;
      ...
      ioEltOut->Timestamp() = ts;
      ioEltOut->VectorSize() = static_cast<int>(structCount * fieldSize *
        field.capacity);
      StopWriting(ioEltOut);
    }
  };
...
ComponentClass::OutputWrapper ComponentClass::createOutput(const
  acc_rtmaps_tlc_types::Field& field)
{
  const auto newOutput = [this] (const std::string& modelNameBase, const std::
...
#define _return_if(type_name)          if (field.type == #type_name) { return makePrimitiveOutputWrapper<type_name>( newOutput(mapPrimitiveType<type_name>(), field.name), field ); }
  ...
  {
    MAPSOutput* output = &NewOutput("o_struct", MAPSString("o_") +
    field.name.c_str());
...
void ComponentClass::checkInputType(const MAPSTypeInfo& inputTypeInfo)
{
  ...
  if (inputTypeName == nullptr) {
    ...
    Error(MAPSStreamedString() << "Input type mismatch [expected:" <<
          m_input_type.c_str() << "] [received:" << (*inputTypeName) << "]");
  }
}
...
void ComponentClass::writeOutputs(const uint8_t* inputStructs, const size_t
  inputStructCount, const MAPSTimestamp ts)
{
  for (auto& outputWrapper : m_outputList) {
